...
impl Header {
    ...
    pub fn new(header_data: &[u8]) -> Result<Self, HeaderError> {
        ...
        let mut location =
            Location::find(codes.copy_byte(0), codes.copy_byte(1), Some(Version::ER12));
        ...
        if location.is_none() {
            ...
            if location.is_none() {
                location =
                    Location::find(codes.copy_byte(0), codes.copy_byte(1), Some(Version::ER10));

                if location.is_none() {
                    location = Location::find(
                        codes.copy_byte(0),
                        codes.copy_byte(1),
                        Some(Version::Vanilla),
                    );
                }
            }
        }
